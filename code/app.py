...
def home():
    ...
    if request.cookies.get("loggedIn")=="1":
        user_id=Users.query.filter(Users.user_name == request.cookies.get("username")).first().user_id
        playlists=Playlists.query.filter(Playlists.playlist_owner_id == user_id).all()
        return render_template('index.html',top_rated_songs=top_rated_songs,playlists=playlists,nav_username=request.cookies.get("username"),loggedIn_status=request.cookies.get("loggedIn"),playlist_url=playlist_url,album_url=album_url,albums=albums)
    return render_template('index.html',top_rated_songs=top_rated_songs,nav_username=request.cookies.get("username"),loggedIn_status=request.cookies.get("loggedIn"),playlist_url=playlist_url,album_url=album_url,albums=albums)
...
def create(user_song_count=0):
    if request.cookies.get("loggedIn") == "1" and request.cookies.get("role") == "creator":
        ...
        return render_template('create.html',nav_username=request.cookies.get("username"),albums=user_albums,user_album_count=user_album_count,user_song_count=user_song_count)
    elif request.cookies.get("loggedIn") == "1" and request.cookies.get("role") == "user":
        if request.method == 'GET':
            return render_template('before_create.html',nav_username=request.cookies.get("username"))
...
@app.route('/playlist_songs/',methods=['POST'])
@app.route('/playlist_songs/<string:playlist_name>',methods=['GET'])
def playlist_songs(playlist_name=None):
    if request.method=='GET' and playlist_name is not None:
        ...
        return render_template('songs.html',nav_username=request.cookies.get('username'),data=1,songs=songs,playlists=playlists,name=playlist_name)
...
@app.route('/album_songs/<string:album_name>',methods=['GET'])
def album_songs(album_name=None):
    if request.method=='GET'and album_name is not None:
        ...
        return render_template('songs.html',nav_username=request.cookies.get('username'),data=0,songs=songs,name=album_name,user_playlists=user_playlists)
...
@app.route('/creator_song_edit',methods=['POST'])
def creator_song_edit():
    ...
    with app.app_context():
        try:
            song_update=Songs.query.filter(Songs.song_name == song_name).first()
            song_update.song_lyrics=lyrics
            song_update.genre=genre
            song_update.genre=genre
            song_update.song_file = audio_file_io
            db.session.add(song_update)
            db.session.commit()
        except:
            db.session.rollback()
    return f"""<html>
...
def add_song_queue(song_name):
    ...
    album_owner_name=Users.query.filter(Users.user_id == album_owner_id).first().user_name
    ...
    if request.method == "POST":
        ...
        if username in admin_dict.keys():
            ...
            if password == user_password:
                ...
                response.set_cookie("admin_u", username)
                return response
            else:
                return f"""<html>
                        <head>
...
def admin_logout():
    ...
    response.set_cookie("admin_u", "none")
    return response
...
def admin_index_page():
    ...
    return render_template('admin_index.html', nav_username=request.cookies.get("admin_u"), total_users = total_users, total_creators = total_creators, total_songs= total_songs, total_albums = total_albums)
...
def show(ele):
    ...
    if ele == 'user':
        ...
        return render_template('details_admin.html', data=all_data, check = "user",nav_username=request.cookies.get("admin_u"))
    elif ele == 'creator':
        ...
        for row in data:
            ...
            temp.append(name)
            all_data.append(temp)
        return render_template('details_admin.html', data=all_data, check = "creator",nav_username=request.cookies.get("admin_u"))
    elif ele == 'songs':
        ...
        for row in data:
            ...
            album_owner=Users.query.filter(Users.user_id == row.album.album_owner_id).first().user_name
            ...
            all_data.append(temp)
        return render_template('details_admin.html', data=all_data, check = "songs",nav_username=request.cookies.get("admin_u"))
    elif ele=='albums':
        ...
        for row in data:
            ...
            temp.append(album_name)
            album_owner = Users.query.filter(Users.user_id == row.album_owner_id).first().user_name
            temp.append(album_owner)
            ...
            all_data.append(temp)
        return render_template('details_admin.html', data=all_data, check = "albums",nav_username=request.cookies.get("admin_u"))
